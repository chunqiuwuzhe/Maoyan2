package maoyanceshi.pjs.com.recyclerrefershdemo.view;
 ...
 */
public class PRecycleview extends RecyclerView {
    /**
     * 是否可以刷新的常量
     */
    ...
    //当前的头布局
    private PRecycleviewRefreshHeadView mHeadView;
    //监听器
    ...
    public PRecycleview(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PRecycleview(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    private void init() {
        ...
        if (pullRefreshEnabled) {
            //获取头布局
            PRecycleviewRefreshHeadView refreshHeader = new PRecycleviewRefreshHeadView(getContext());
            //添加进去
            mHeaderViews.add(0, refreshHeader);
            mHeadView = refreshHeader;
        }
        PRecycleviewRefreshFootView footView = new PRecycleviewRefreshFootView(getContext());
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && refreshAndLoadMoreListener != null && !isLoadingData && loadMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                    ...
                    && !isNoMore && mHeadView.getStatus() < PRecycleviewRefreshHeadView.STATE_REFRESHING) {

                View footView = mFootViews.get(0);
                isLoadingData = true;
                if (footView instanceof PRecycleviewRefreshFootView) {
                    ((PRecycleviewRefreshFootView) footView).setState(PRecycleviewRefreshFootView.STATE_LOADING);
                } else {
        ...
        public void onChanged() {
            ...
            if (adapter != null && mEmptyView != null) {
                ...
                if (adapter.getItemCount() == emptyCount) {
                    ...
                    PRecycleview.this.setVisibility(View.GONE);
                } else {
                    mEmptyView.setVisibility(View.GONE);
                    PRecycleview.this.setVisibility(View.VISIBLE);
                }
            }
    ...
    public void setNoMoreData(boolean noMore) {
        ...
        View footView = mFootViews.get(0);
        ((PRecycleviewRefreshFootView) footView).setState(isNoMore ? PRecycleviewRefreshFootView.STATE_NOMORE : PRecycleviewRefreshFootView.STATE_COMPLETE);
    }
    ...
    public void setloadMoreComplete() {
        ...
        View footView = mFootViews.get(0);
        if (footView instanceof PRecycleviewRefreshFootView) {
            ((PRecycleviewRefreshFootView) footView).setState(PRecycleviewRefreshFootView.STATE_COMPLETE);
        } else {
    ...
    public void setRefreshing(boolean refreshing) {
        if (refreshing && pullRefreshEnabled && refreshAndLoadMoreListener != null) {
            mHeadView.setState(PRecycleviewRefreshHeadView.STATE_REFRESHING);
    ...
    public void addHeadView(View headView) {
        if (pullRefreshEnabled && !(mHeaderViews.get(0) instanceof PRecycleviewRefreshHeadView)) {
            PRecycleviewRefreshHeadView refreshHeader = new PRecycleviewRefreshHeadView(getContext());
            mHeaderViews.add(0, refreshHeader);
            mHeadView = refreshHeader;
        }
     ...
     * @param footView
     */
    private void addFootView(PRecycleviewRefreshFootView footView) {
        mFootViews.clear();
